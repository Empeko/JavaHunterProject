...
def parse_to_vacancy_model(json_vacancy):
    ...
    #vacancy.date_publication = datetime.strptime(json_vacancy['published_at'], '%Y-%m-%dT%H:%M:%S%z')
...
def get_vacancies_urls(count):
    params = {
        'text': 'Java программист',  # Текст фильтра. В имени должно быть слово "Аналитик"
        'area': 113,  # Поиск ощуществляется по вакансиям города Москва
        'page': 0,  # Индекс страницы поиска на HH
        'per_page': count  # Кол-во вакансий на 1 странице
    }
